...
function unrotateState(rotatedState, mySlotIndex) {
    ...
    // FIX: Rotate the Round Winner so the other team sees the exact correct perspective!
    if (rotatedState.roundEndedBy && rotatedState.roundEndedBy !== 'deck-empty') {
        const localEndIndex = compass.indexOf(rotatedState.roundEndedBy);
        unrotated.roundEndedBy = compass[(localEndIndex + mySlotIndex) % 4];
    }

    // FIX: Properly rotate the Team Open status across the network!
    if (mySlotIndex === 1 || mySlotIndex === 3) {
        const tempUs = rotatedState.isOpen.us;
        unrotated.isOpen.us = rotatedState.isOpen.them;
        unrotated.isOpen.them = tempUs;
    }
    return unrotated;
}
...
function broadcastState() {
    ...
    // Create a deep copy of the state before sending it
    const stateToSend = isHost ? JSON.parse(JSON.stringify(roundState)) : unrotateState(roundState, mySlotIndex);

    // PRIVACY LOCK: Scrub ALL pending melds from the payload! 
    // This ensures opponents only see your cards AFTER you confirm your opening meld.
    stateToSend.pendingMelds = { player: {}, east: {}, north: {}, west: {} };

    socket.emit('syncGameState', { roomCode: currentRoom, state: stateToSend });
}

// AUTO-SYNC: Host updates Guests every second
setInterval(() => {
    if (isMultiplayer && isHost) {
        // PAUSE auto-sync if the Host is actively drafting a bench take!
        if (roundState.activePlayer === 'player' && roundState.pendingBenchTake) return;
        broadcastState();
    }
}, 1000);
    ...
    if (!socket) {
        ...
        socket.on('gameStarted', (players) => {
            ...
            if (isHost) {
                ...
                roundState.turnPhase = 'DEALING'; // FIX: Force Guest into DEALING phase initially so they don't lock up!
            }
        });
        ...
        socket.on('playerMoved', (data) => {
            ...
            if (data.type === 'deal') {
                ...
                audio.playSFX('success');
            } else if (data.type === 'drawRed3') {
                roundState.red3s[localPlayerKey].push(data.card);
                renderDeployedRed3s(localPlayerKey);
                if (roundState.deck.length > 0) roundState.deck.pop();
                updateDeckCount();
                audio.playSFX('success');
                ...
                animateCardThrow(localPlayerKey, data.card, null, () => {
                    ...
                    // FIX: Make the Host instantly verify if this discard emptied the Guest's hand to end the game!
                    if (isHost && roundState.hands[localPlayerKey] && roundState.hands[localPlayerKey].length === 0) {
                        checkRoundEnding(localPlayerKey);
                    }
                });
                audio.playSFX('card-flip');
        ...
        socket.on('gameStateUpdated', (absoluteState) => {
            ...
            if (!isHost) {
                ...
                // FIX: Decrypt the Round Winner rotation so Guests accurately calculate the opponent's 100 bonus points!
                if (absoluteState.roundEndedBy && absoluteState.roundEndedBy !== 'deck-empty') {
                    const absoluteEndIndex = compass.indexOf(absoluteState.roundEndedBy);
                    rotatedState.roundEndedBy = compass[(absoluteEndIndex - mySlotIndex + 4) % 4];
                }
            }

            // DRAFT PROTECTION: If it is your turn, protect hand and drafts from server overwrites!
            if (roundState.activePlayer === 'player' && roundState.turnPhase !== 'DEALING') {
                rotatedState.hands.player = roundState.hands.player;
                rotatedState.pendingMelds.player = roundState.pendingMelds.player;
                rotatedState.red3s.player = roundState.red3s.player;
                rotatedState.discardPile = roundState.discardPile;

                // FIX: ONLY lock the turn phase if you are actively hiding a drafted bench card
                if (roundState.pendingBenchTake) {
                    rotatedState.pendingBenchTake = roundState.pendingBenchTake;
                    rotatedState.pulledBenchCard = roundState.pulledBenchCard;
                    rotatedState.turnPhase = roundState.turnPhase;
                }
            }
...
function startNewRound() {
    ...
    if (isMultiplayer && isHost) broadcastState(); // FIX: Instantly tell Guests a new round is dealing!
...
function animateDealing(totalToGive, callback) {
    ...
    function dealNextCard() {
        ...
        if (allDone) {
            setTimeout(() => {
                callback(); // Unlock the turn phase to MELD_PHASE first!
                if (isMultiplayer && isHost) broadcastState(); // Then broadcast the active turn to all guests
            }, 500);
        } else {
...
function createCardFlyAnimation(destinationPlayer) {
    ...
    const scale = rectTable.width / tableArea.offsetWidth;

    const temp = document.createElement('div');
    temp.className = 'dealing-card-temp';
    temp.style.top = `${(rectStart.top - rectTable.top) / scale}px`;
    temp.style.left = `${(rectStart.left - rectTable.left) / scale}px`;

    tableArea.appendChild(temp);
    temp.getBoundingClientRect();

    temp.style.top = `${(rectEnd.top - rectTable.top) / scale + 15}px`;
    temp.style.left = `${(rectEnd.left - rectTable.left) / scale + 20}px`;
...
function animateCardThrow(sourceKey, card, domSourceNode, callback) {
    ...
    // Calculate exact scale to fix animation targeting!
    const scale = rectTable.width / tableArea.offsetWidth;
    ...
    if (domSourceNode) {
        ...
        startX = (rectSource.left - rectTable.left) / scale;
        startY = (rectSource.top - rectTable.top) / scale;
    } else {
        ...
        if (seatEl) {
            ...
            startX = (rectSeat.left + (rectSeat.width / 2) - rectTable.left) / scale - 30; // 30 is half width
            startY = (rectSeat.top + (rectSeat.height / 2) - rectTable.top) / scale - 45; // 45 is half height
        } else {
            startX = 0; startY = 0;
        }
    }
    ...
    flyCard.style.width = '60px';  // NEW SIZE
    flyCard.style.height = '90px'; // NEW SIZE
    ...
    if (card.rank === 'JK') {
        ...
        flyCard.innerHTML = `<div class="card-top">2</div><div class="card-center" style="display:flex; flex-direction:column; align-items:center;"><span>${card.suit}</span><span style="font-size:0.8em">🃏</span></div><div class="card-bottom">2</div>`;
    } else {
    ...
    const targetX = (rectDiscard.left - rectTable.left) / scale;
    const targetY = (rectDiscard.top - rectTable.top) / scale;
...
function animateBenchTakeAction(playerKey, rank, requiredCount) {
    ...
    const seatX = (rectSeat.left + (rectSeat.width / 2) - rectTable.left) / scale - 30;
    const seatY = (rectSeat.top + (rectSeat.height / 2) - rectTable.top) / scale - 45;
    const discardX = (rectDiscard.left - rectTable.left) / scale;
    const discardY = (rectDiscard.top - rectTable.top) / scale;
    ...
    for (let i = 0; i < requiredCount; i++) {
        ...
        cardDiv.style.width = '60px';  // NEW SIZE
        cardDiv.style.height = '90px'; // NEW SIZE
        ...
        cardDiv.innerHTML = `<div class="card-top">${rank}</div><div class="card-center">${suit}</div><div class="card-bottom">${rank}</div>`;
        ...
        setTimeout(() => { audio.playSFX('card-flip'); }, 300 + i * 50);
    }
...
function renderPlayerHand() {
    ...
    // AUTO-SORT: Automatically arrange the hand by rank every time it renders!
    ...
    roundState.hands.player.forEach(card => {
        const cardDiv = document.createElement('div');
        const suitCls = getSuitClass(card.suit);
        cardDiv.className = `card ${suitCls} ${card.rank === 'JK' || card.rank === '2' ? 'card-joker' : ''}`;
        ...
        if (card.isNew) {
            cardDiv.classList.add('new-draw-glow');
        }
        ...
        if (card.rank === 'JK') {
            ...
            cardDiv.innerHTML = `
                ...
                <div class="card-center" style="display:flex; flex-direction:column; gap:2px; font-size:1.1rem; align-items:center;"><span>${card.suit}</span><span>🃏</span></div>
...
function renderDiscardPile() {
    ...
    // DYNAMIC SIZING: Calculate scale based on the number of cards in the pile
    const count = roundState.discardPile.length;
    let pileScale = 1.4 - (count * 0.015); // Start at 140% size, shrink slightly per card
    if (pileScale < 0.8) pileScale = 0.8; // Cap the minimum size so it doesn't get too small
    pile.style.transform = `scale(${pileScale})`;
    ...
    const topCard = roundState.discardPile[count - 1];
    pile.className = 'discard-pile has-card';

    const suitCls = getSuitClass(topCard.suit);
    const cardDiv = document.createElement('div');
    cardDiv.className = `card ${suitCls} ${topCard.rank === 'JK' || topCard.rank === '2' ? 'card-joker' : ''}`;
    ...
    if (topCard.rank === 'JK') {
        ...
        cardDiv.innerHTML = `
            <div class="card-top">2</div>
            <div class="card-center" style="display:flex; flex-direction:column; align-items:center;"><span>${topCard.suit}</span><span style="font-size:0.8em">🃏</span></div>
            <div class="card-bottom">2</div>
        `;
    } else {
        cardDiv.innerHTML = `
    ...
    // --- RENDER THE DISCARD COUNTER BADGE ---
    const countBadge = document.createElement('div');
    countBadge.className = 'discard-count-badge';
    countBadge.textContent = count;

    // Reverse the scaling on the text so the numbers remain readable even as the pile shrinks!
    countBadge.style.transform = `scale(${1 / pileScale})`;
...
const meldAreaPlayer = document.getElementById('meld-area-player');
if (meldAreaPlayer) {
    meldAreaPlayer.onclick = (e) => {
        // Prevent accidental clicks if no cards are selected
        if (selectedCards.length > 0) {
...
document.getElementById('btn-confirm-meld').onclick = () => {
    ...
    if (isMultiplayer) broadcastState(); // FIX: Sync confirmed melds immediately to the server so they don't disappear!
};
...
function renderPlayerMeldsHTML(playerKey, meldObject, pendingObject, containerId) {
    ...
    combinedRanks.forEach(rank => {
        ...
        cardsList.forEach((card, index) => {
            ...
            // Dynamically adjust size/overlap for the NEW LARGER table cards
            let overlap = 32;
            if (cardsList.length >= 5) overlap = 42;
            if (cardsList.length >= 7) overlap = 52;
            ...
            if (card.rank === 'JK') {
                ...
                cardDiv.innerHTML = `
                    <div class="card-top">2</div>
                    <div class="card-center" style="display:flex; flex-direction:column; align-items:center;"><span>${card.suit}</span><span style="font-size:0.8em">🃏</span></div>
                    <div class="card-bottom">2</div>
                `;
            } else {
                cardDiv.innerHTML = `
...
function setupPlayerDrawPrompt() {
    ...
    if (isStarterNoDraw) {
        ...
        document.getElementById('draw-pile').onclick = null; // FIX: Completely disable draw pile for starting player!
    } else {
        roundState.turnPhase = 'DRAW_PHASE';
        highlightDrawDeck(true);
        ...
        drawPile.onclick = () => {
            ...
            if (drawn) {
                drawn.isNew = true; // Mark the card to glow blue!
                ...
                // Remove the blue glow automatically after 2 seconds
                setTimeout(() => {
                    drawn.isNew = false;
                    renderPlayerHand();
                }, 2000);
...
document.getElementById('btn-take-bench').onclick = () => {
    ...
    if (isOpen) {
        ...
        // PENDING DRAFT MODE - Hide top card locally, DO NOT Broadcast!
        ...
        if (!isStarterNoDraw) {
            ...
            if (!tookBench) {
                ...
                if (drawn) {
                    ...
                    createCardFlyAnimation(botKey); // FIX: Ensure Host sees bot drawing animation!
                    if (isMultiplayer) {
                        socket.emit('sendMove', {
                            roomCode: currentRoom,
                            type: 'draw',
...
function executeBotDiscard(botKey) {
    ...
    if (cardToDiscard) {
        ...
        // Wait for the physical animation to finish BEFORE adding to pile and advancing turn!
        animateCardThrow(botKey, cardToDiscard, null, () => {
...
function triggerRoundEnd(endedByPlayerKey) {
    audio.playSFX('success');
    roundState.roundEndedBy = endedByPlayerKey;

    const scores = calculatePoints();
    globalScores.us += scores.usRound;
    globalScores.them += scores.themRound;

    const isRoundSkipped = (endedByPlayerKey === 'deck-empty' && scores.usRound === 0 && scores.themRound === 0);
    ...
    if (isRoundSkipped) {
        ...
        document.getElementById('score-lines-them').innerHTML = buildScoreLines(
            themNoCanasta ? [] : themMeldCollections,
            themNoCanasta ? [] : themHand,
            themRed3s,
    ...
    if (endedByPlayerKey === 'player' || endedByPlayerKey === 'north') title = "Your Team Went Out!";
    else if (endedByPlayerKey === 'west' || endedByPlayerKey === 'east') title = "Opponents Went Out!";
    else if (endedByPlayerKey === 'deck-empty') title = "Deck Empty — Round Skipped";
    document.getElementById('round-summary-title').textContent = title;
